namespace L12_Abschlussaufgabe {
    ...
    let chocolateX: number = Math.random() * can.width;
    let chocolateY: number = -100; //damit es breits über Canvas gezeichnet wird
    let carrotX: number = Math.random() * can.width;
    ...
    const hase = new Bild(50, can.height + 250, 120, 150, 'image/hase.png');
    const chocolate = new Bild(Math.random() * can.width, -100, 65, 65, 'image/Schoko.png');
    const carrot = new Bild(Math.random() * can.width, -100, 65, 65, 'image/karotte..png');


    //Canvas Größe
    window.onload = () => {
        can.height = window.innerHeight - 100; // -100, da Platz für Buttons benötigt wird.
        can.width = window.innerWidth - 20; // sonst reicht Bildschirm nicht?
    ...
    function updatePosition(): void {
        if (chocolateY > can.height) {
            chocolateY = -100;
            chocolateX = Math.random() * can.width;
        }
        if (carrotY > can.height) {
            carrotY = -100;
            carrotX = Math.random() * can.width;
        }
        if (carrotX >= can.width - 50) {
            carrotX = can.width - 55;
        }
    }
